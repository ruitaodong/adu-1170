...
void enable_image(void)
{
    ...
    status = sbl_flash_write(off, (void *)&s_remap_trailer.magic, IMAGE_TRAILER_SIZE - 16);
    if (status) 
    {
...
void write_image_ok(void)
{
    ...
    sbl_flash_init();
    sbl_flash_read(REMAP_FLAG_ADDRESS, &s_remap_trailer, 32);
    
    primask = DisableGlobalIRQ();
    status = sbl_flash_erase(0xff000, 0x1000);
    ...
    primask = DisableGlobalIRQ();
    status = sbl_flash_write(off, (void *)&s_remap_trailer, IMAGE_TRAILER_SIZE);
    if (status) 
    {
        return;
    }
...
void enable_image(void)
{
    ...
    primask = DisableGlobalIRQ();
#ifdef SOC_LPC55S69_SERIES
    status = sbl_flash_write(off, write_buff, 512);
#else
    status = sbl_flash_write(off, (void *)&s_swap_trailer, IMAGE_TRAILER_SIZE);
#endif
    if (status) 
    {
